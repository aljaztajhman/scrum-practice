...
function InfiniteSession({
  ...
  useEffect(() => {
    scrollTop();
  }, [engine.history.length, engine.stage]);
  ...
  useEffect(() => {
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Escape') {
        ...
        if (i < engine.current.options.length) {
          e.preventDefault();
  ...
  if (engine.stage === 'results') {
    ...
    engine.history.forEach((h, i) => {
      answers[i] = { selected: h.selected, correct: h.correct };
    });
    return (
      <PageShell>
    ...
    <PageShell>
      {showExitConfirm && (
        <ExitModal
          ...
          total={engine.answeredCount}
          onCancel={() => setShowExitConfirm(false)}
          onConfirm={() => {
            setShowExitConfirm(false);
            engine.stopAndReview();
          }}
        />
      )}
        ...
        <div className="flex items-center justify-between mb-4 gap-2">
          ...
          <div className="flex items-center gap-2.5 md:gap-4 text-xs">
            ...
            <span className="flex items-center gap-1 text-emerald-800">
              ...
              <span className="tabular-nums font-medium">{engine.correctCount}</span>
            </span>
