  ...
  return (
    <div>
      <div className="bg-white/70 backdrop-blur-sm border border-stone-300 p-6 md:p-10 paper">
        ...
        <h2 className="serif text-2xl md:text-3xl text-stone-900 leading-snug mb-8" style={{ fontWeight: 500 }}>
          {question.q}
        </h2>
          ...
          {question.options.map((opt, i) => {
            ...
            return (
              <button
                ...
                disabled={locked}
                aria-pressed={isSel}
                className={`w-full text-left border-l-4 border-y border-r px-4 md:px-5 py-3.5 transition-all duration-150 flex items-start gap-3 ${cls} ${locked ? 'cursor-default' : 'cursor-pointer'}`}
              >
                <span
                  className={`shrink-0 w-6 h-6 border flex items-center justify-center mt-0.5 text-xs ${
                    showAsCorrect ? 'border-emerald-700 bg-emerald-700 text-white'
                    : showAsWrong ? 'border-rose-700 bg-rose-700 text-white'
                    : isSel ? 'border-stone-900 bg-stone-900 text-white'
                    : 'border-stone-400 text-transparent'
                  }`}
                  style={{ borderRadius: question.type === 'multi' ? '2px' : '50%' }}
                >
                  {showAsCorrect ? <Check className="w-3.5 h-3.5" strokeWidth={3} />
                    : showAsWrong ? <X className="w-3.5 h-3.5" strokeWidth={3} />
        ...
        {locked && (
          <div role="status" aria-live="polite" className={`mt-7 border-l-4 pl-5 py-4 pr-5 slide-up ${isCorrect ? 'border-emerald-700 bg-emerald-50/60' : 'border-rose-700 bg-rose-50/60'}`}>
      ...
      <div className="mt-5 flex items-center justify-between gap-3">
        ...
        <span className="text-xs text-stone-500 uppercase tracking-wider hidden sm:block">
          {question.type === 'multi' && !locked && selected.length > 0 ? `${selected.length} selected` : ''}
        </span>
        {!locked ? (
          <button onClick={onSubmit} disabled={!canSubmit} className="bg-stone-900 text-stone-50 px-6 py-3 text-xs md:text-sm uppercase tracking-widest hover:bg-amber-700 disabled:bg-stone-300 disabled:cursor-not-allowed transition-colors">
            Submit answer
          </button>
        ) : (
