...
export default function Results({
  ...
  if (isReview) {
    ...
    subtitle =
      missed.length === 0
        ? `${correct} out of ${total}. Clean sweep — every weak question got another correct streak.`
        : `${correct} out of ${total} from your weak-spot queue. Two correct in a row retires a question from review.`;
    primaryCtaLabel = 'Review again — refresh queue';
    secondaryCtaLabel = 'Back to home';
  } else {
    eyebrow = passed ? 'On track' : 'Keep studying';
    subtitle = `${correct} correct out of ${total}. ${
      passed
        ? "That's at or above the 85% threshold."
        : `You need ${Math.ceil(total * 0.85) - correct} more correct to reach 85%.`
    }`;
    primaryCtaLabel = `New round — same ${track.title}`;
    secondaryCtaLabel = 'Change certification / length';
  }

  return (
    <div className="space-y-10 md:space-y-12">
      <div className="border border-stone-300 bg-white/70 p-8 md:p-12 paper">
        <p className="text-xs uppercase tracking-[0.3em] text-stone-600 mb-4">{eyebrow}</p>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        >
          <RotateCcw className="w-4 h-4" strokeWidth={1.8} /> {primaryCtaLabel}
        </button>
        <button
          onClick={onRestart}
        ...
        {visibleResults.length === 0 ? (
          ...
          <div className="space-y-4">
            {visibleResults.map((r, i) => (
              <div
                key={i}
                className={`border-l-4 bg-white/60 p-5 ${
                  r.correct ? 'border-emerald-700' : 'border-rose-700'
                }`}
              >
