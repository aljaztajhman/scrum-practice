...
export default function TopicPicker({ track, onPick }: Props) {
  ...
  const topics = Object.keys(track.topicAccent).filter((tp) => (counts[tp] ?? 0) > 0);
        ...
        {topics.map((topic) => {
          ...
          return (
            <button
              ...
              className="group text-left border border-stone-400 bg-white/50 hover:border-stone-900 hover:bg-white/80 p-5 md:p-6 transition-all duration-200"
            >
              <div className="flex items-start justify-between gap-3 mb-3">
                <span className={`text-[10px] uppercase tracking-widest px-2 py-1 border ${accent}`}>
                  {topic}
                </span>
                <span className="text-xs text-stone-500 tabular-nums">{n} Q</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="serif text-2xl text-stone-900" style={{ fontWeight: 500 }}>
                  {t('topic.drillIt')}
                </span>
